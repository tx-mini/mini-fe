...
class FirstSlide extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        this.setState({classDir: nextProps.classDir, brushList: nextProps.brushList})
    }
    componentDidMount() {
        ...
        this.setState({showCChild: initC, showBChild: initB})
    }
    toggleShow = (index, type) => () => {
        ...
        if (!type) {
            ...
            this.setState({showCChild: arr})
        } else {
            arr = showBChild
            arr[index] = !showBChild[index];
            this.setState({showBChild: arr})
        }
    }
    selectItem = (i, index, data, type) => () => {
        ...
        this.setState({curCIndex: curCIndex, curCParentIndex: curCParentIndex, curBIndex: curBIndex, curBParentIndex: curBParentIndex})
        //传递选中的data
    ...
    render() {
        ...
        return (
            <div className="first-slide">
                <div className="new-note" onClick={this.createNote}>
                    ...
                    <div className="class-note">
                        ...
                        {classDir.map((item, index) => (
                            <div className={showClass? "show" : "hide"} key={index}>
                                <div className="slide-item sub-item" onClick={this.toggleShow(index, false)}>
                                ...
                                </div>
                                {
                                    item.childrens.map((data, i) => (
                                        <div className={["slide-item final-item", showCChild[index]? "final-show" : "hide"].join(" ")}
                                            onClick={this.selectItem(i, index, data, false)}
                                            key={i}>
                        ...
                        </div>
                        {
                            brushList.map((item, index) => (
                                <div className={showDrush? "show" : "hide"} key={index}>
                                    <div className="slide-item sub-item" onClick={this.selectItem("", index, item, true)}>
                                        <i className="iconfont icon-wenben show-icon item-icon"></i>
